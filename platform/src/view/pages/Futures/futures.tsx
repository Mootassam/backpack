// src/components/Futures.tsx (optimized with i18n and updated template CSS)
...
function Futures() {
  ...
  useEffect(() => {
    ...
    const connectTickerWebSocket = () => {
      ...
      try {
        ...
        tickerWs.current.onerror = (error: Event) => console.error("Ticker WebSocket error:", error);
  ...
  return (
    <div className="container">
      ...
      {/* Updated CSS to match Profile template */}
      <style>{`
        * {
          margin: 0;
          padding: 0;
...
// Extracted Order Detail Modal Component (unchanged)
  ...
  <div className="modal-overlays" onClick={onClose}>
    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="modal-body">
          ...
          <div className="detail-header">
            ...
            <span
              className={`detail-direction ${
                selectedOrder.futuresStatus === "long" || selectedOrder.direction === "BUY UP"
                  ? "buy"
                  : "sell"
              }`}
            >
        ...
        <div className="order-detail-section">
          <OrderDetailRow
            label={i18n('pages.futures.orderDetails.futuresAmount')}
            value={`${selectedOrder.futuresAmount || selectedOrder.investment} USDT`}
          />

          {selectedOrder.contractDuration && (
            <OrderDetailRow
              label={i18n('pages.futures.orderDetails.contractDuration')}
              value={`${selectedOrder.contractDuration} ${i18n('pages.futures.orderDetails.seconds')}`}
            />
          )}
          ...
          <OrderDetailRow
            ...
            value={
              selectedOrder.closePositionTime
                ? i18n('pages.futures.orderDetails.completed')
                : i18n('pages.futures.orderDetails.open')
            }
          />
          <OrderDetailRow
          ...
          {selectedOrder.closePositionPrice && (
            <OrderDetailRow
              label={i18n('pages.futures.orderDetails.closePositionPrice')}
              value={selectedOrder.closePositionPrice}
            />
          )}
          ...
          <OrderDetailRow
            label={i18n('pages.futures.orderDetails.leverage')}
            value={`${selectedOrder.leverage}X`}
          />
        </div>
      </div>
...
// Helper component for order detail rows (unchanged)
